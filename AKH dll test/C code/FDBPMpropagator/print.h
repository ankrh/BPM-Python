...
//  #include <complex>
  ...
  #define print(x);        printScl(__LINE__,#x,x);
  #define printArray(x,N); printArr(__LINE__,#x,x,(int)N);
  ...
  void printScl(int line, char const *name, std::complex<float>x) {printf("%d float complex %s:    %g%+gi\n", line, name, std::real(x), std::imag(x));}
  void printScl(int line, char const *name, double             x) {printf("%d double %s:    %lg\n" ,line,name,x);}
  void printScl(int line, char const *name, std::complex<double>x) { printf("%d double complex %s:    %g%+gi\n", line, name, std::real(x), std::imag(x)); }
  void printScl(int line, char const *name, long double        x) {printf("%d long double %s:    %Lg\n" ,line,name,x);}
  void printScl(int line, char const *name, std::complex<long double>x) { printf("%d long double complex %s:    %g%+gi\n", line, name, std::real(x), std::imag(x)); }
  ...
  void printArr(int line, char const* name, std::complex<float>* x, int N) {
      printf("%d float complex array %s at %llu:   ", line, name, x);
      if (N > 0) {
          for (int i = 0;i < N;i++) {
              if (i < PRINTLIMIT) {
                  printf(" %g%+gi", std::real(x[i]), std::imag(x[i]));
              }
          }
          printf("\nlength: %d\n", N);
      }
  ...
  void printArr(int line, char const* name, std::complex<double>* x, int N) {
      printf("%d double complex array %s at %llu:   ", line, name, x);
      if (N > 0) {
          for (int i = 0;i < N;i++) {
              if (i < PRINTLIMIT) {
                  printf(" %g%+gi", std::real(x[i]), std::imag(x[i]));
  ...
  void printArr(int line, char const* name, std::complex<long double>* x, int N) {
      printf("%d long double complex array %s at %llu:   ", line, name, x);
      if (N > 0) {
          for (int i = 0;i < N;i++) {
              if (i < PRINTLIMIT) {
                  printf(" %g%+gi", std::real(x[i]), std::imag(x[i]));
